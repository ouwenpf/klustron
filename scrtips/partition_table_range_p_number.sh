...
first_partition_table_name="${base_table_name}_p0"
...
if [ "$partition_type" == "yearly" ]; then
  ...
  for i in $(seq 1 $((num_partitions - 1 + 1))); do
    shard_index=$(($i % ${#shard[@]}))
    shard_number=${shard[shard_index]}

  
	if [ "$current_year" -lt "$(date -d "$end_date" +%Y)" ]; then
		next_year=$((current_year + 1))
		partition_table_name="${base_table_name}_p$i"
...
FOR VALUES FROM ('${current_year}-${start_date:5:5}') TO ('${next_year}-${start_date:5:5}') WITH (shard = ${shard_number}) ;"
  ...
  # 输出最后一个分区表SQL语句
  last_partition_table_name="${base_table_name}_p$(($i + 1))"
...
FOR VALUES FROM ('$end_date') TO (MAXVALUE);"
  ...
  if [ "$(date -d "$current_date" +%Y%m)" != "$(date -d "$end_date" +%Y%m)" ]; then
...
FOR VALUES FROM ('$end_date') TO (MAXVALUE);"
  ...
  if [ "$(date -d "$current_date" +%Y%m%d)" != "$(date -d "$end_date" +%Y%m%d)" ]; then
