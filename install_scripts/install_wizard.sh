...
if [[ -f "/etc/os-release" ]]; then
    source /etc/os-release
      ...
      if [[ "$ID" == "ubuntu" ]]; then
        echo  -e "$COL_START${GREEN}正在安装必须的软件包,否则安装脚本无法进行.....$COL_END"
        ...
        echo  -e "$COL_START${GREEN}正在检查配置信息,否则安装脚本无法进行.....$COL_END"
        for i in figlet expect dos2unix jq
        do
           if ! command -v "$1" &> /dev/null; then
...
while true; do
  ...
  2.程序目录和下载:建议创建目录/softwares/
                   cd /softwares/ && git clone https://gitee.com/zettadb/cloudnative.git
  3.编辑配置文件：
...
while true; do
  ...
  echo -e "$COL_START${GREEN}机器都初始化成功按回车键继续安装程序退出请输入q|Q$COL_END" 
    read -t 300 -rsn1 input
